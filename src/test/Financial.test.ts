...
it('ammortamento() should give the correct ammortamento', () => {
    ...
    const tasso = 1;
    const durata = 2;
    const inizio = moment.now();
    const estinzioni = [];

    const expectedAmmortamento = [
        {
            capitaleVersato: 49.979175343611836,
            date: moment(inizio, 'MM/YYYY').add(2, 'month').format('MM/YYYY'),
        ...
        },
        {
            ...
            date: moment(inizio, 'MM/YYYY').add(3, 'month').format('MM/YYYY'),
    ...
    expect(ammortamento({ tasso, capitale, durata, inizio, estinzioni })).toEqual(
        expectedAmmortamento
    );
});
